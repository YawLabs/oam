...
| `vm.SourceTextModule` | Supported behind `--experimental-vm-modules`: compile, `link` (including async linkers and import graphs), `evaluate`, `namespace`, `status`, `error`, `dependencySpecifiers`, and the `context` option. Not implemented: `vm.SyntheticModule`, `createCachedData`, `cachedData`, `importModuleDynamically`, `initializeImportMeta`, and `evaluate`'s `timeout`/`breakOnSigint` (accepted and ignored, as elsewhere in `vm`). | Same, plus the above. |
