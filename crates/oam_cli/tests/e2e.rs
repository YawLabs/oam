...
/// Windows: the raw-mode switch against a REAL console. UNFINISHED -- see
/// "What is missing" below; behind the `conpty-e2e` feature so it neither
/// runs nor compiles by default.
///
/// What works, proved by `the_pseudoconsole_harness_carries_child_output`:
/// creating a pseudoconsole, attaching a child to it, feeding it keystrokes,
/// and reading back the VT stream it paints.
///
/// What is missing: a console INPUT handle the child can write to. Under
/// `cargo test` the harness's own std handles are redirected into the test
/// runner, and a child spawned the ordinary way comes up holding those -- so
/// its output bypasses the pseudoconsole even though its console IS the pty
/// (the same command writing explicitly to CONOUT$ lands in the pty; clearing
...
/// opens CONIN$ itself and execs the real command, or running this suite from
/// a parent whose std handles are not redirected. Tracked in YawLabs/oam#109.
///
...
/// -- and the `oam_core::stdin` unit tests stop at the state machine. So this
/// module puts `oam run` behind a pseudoconsole (`CreatePseudoConsole` + an
/// `EXTENDED_STARTUPINFO_PRESENT` child, the same mechanism Windows Terminal
/// uses) and drives it through the two paths that only exist on a console:
///
/// - cooked -> raw with a read already pending: one keystroke must arrive as
///   `data` WITHOUT Enter, and the discarded line (the synthetic `\r\n` that
///   woke the cancelled read) must never surface as data;
/// - raw -> cooked with a raw read pending: the cancel must not move the
///   cursor, because a raw read echoes nothing. ConPTY reflects a real
///   `SetConsoleCursorPosition` into the output stream as a cursor escape, so
///   the assertion is that no such escape (and no newline) appears.
#[cfg(all(windows, feature = "conpty-e2e"))]
mod conpty {
    use std::io::{Read, Write};
    ...
    impl ConPty {
        /// `oam run <script> --no-check` on a fresh 120x40 pseudoconsole.
        ...
        /// Run `command` on a fresh 120x40 pseudoconsole, with its stdio
        /// bound to that console.
        ///
        /// Getting the child's STDIO onto the console is the unfinished part
        /// of this harness -- see the module docs. `cmd /c ... <CONIN$
        /// >CONOUT$` is what makes output reach the pty at all under
        /// `cargo test`; its cost is a read-only console INPUT handle.
        fn spawn_command(command: &str) -> Self {
            Self::spawn_raw(&format!("cmd.exe /c \"{command} <CONIN$ >CONOUT$ 2>&1\""))
        }

        /// The command line as given.
        fn spawn_raw(command_line: &str) -> Self {
        ...
        fn wait_for(&self, needle: &str) -> String {
            ...
            loop {
                ...
                assert!(
                    Instant::now() < deadline,
                    "never saw {needle:?} on the console; child {}; raw {:?}; text {text:?}",
                    self.exit_state(),
                    self.raw_output()
                );
        ...
        /// "running" or the exit code, for a failure message.
    ...
    fn the_pseudoconsole_harness_carries_child_output() {
        let pty = ConPty::spawn_command("echo HARNESS_OK");
...
"#,
        ...
        // isRaw is FALSE here today: see the module docs. When the child
        // gets a writable console input handle this becomes "[raw on
        // isRaw=true", and the keystroke assertion below starts meaning
        // something.
        ...
        pty.send(b"h");
        let text = pty.wait_for("[keys [\"h\"]]");
        assert!(
            !text.contains("[keys [\"\\r\\n\"]") && !text.contains("[keys [\"\\r\"]"),
            "the discarded line must never be delivered as data: {text:?}"
        );
        pty.send(b"i");
    ...
    /// Switching back to cooked mode with a RAW read pending must not move the
    /// cursor: a raw read echoes nothing, so there is no newline to undo.
...
"#,
        ...
        // Nothing was written between the two markers, so anything the
        // console emitted there is the cursor being moved.
        let raw = pty.raw_output();
        let start = raw.find("[raw]").expect("marker") + "[raw]".len();
        let end = raw.find("[off]").expect("marker");
        let between = &raw[start..end];
        assert!(
            !between.contains('\n') && !between.contains('\r'),
            "leaving raw mode moved the cursor to a new line: {between:?}"
        );
        // Any CSI whose final byte repositions the cursor.
        for (index, _) in between.match_indices('\u{1b}') {
            let Some(params) = between[index + 1..].strip_prefix('[') else {
                continue;
            };
            let final_byte = params.chars().find(|c| !c.is_ascii_digit() && *c != ';');
            assert!(
                !matches!(final_byte, Some('H' | 'G' | 'A' | 'd' | 'f')),
                "leaving raw mode emitted a cursor-position escape: {between:?}"
            );
        }
    }
}
