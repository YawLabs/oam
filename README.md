...
- [CONFORMANCE-NODE.md](CONFORMANCE-NODE.md) — a vendored subset of Node's own test suite.
  **439/442 pass-over-runnable (99.3%)** on windows-aarch64, the host of the receipt committed
  here; the scorecard is regenerated per host, so a checkout built elsewhere reports its own.
  Read it with [docs/node-divergences.md](docs/node-divergences.md), which qualifies the
  denominator and lists what oam deliberately does differently — all three remaining failures
  are deliberate and named there.
