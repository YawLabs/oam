...
pub(crate) fn run(release: bool) -> Result<()> {
    let repo = repo_root()?;
    // Before the build, so a half-finished Node bump fails in a second. This
    // suite never runs node (the oracle is each test's exit code), so the pin
    // binds the CORPUS rather than the node on PATH -- see node_pin.
    let pinned_node = crate::node_pin::pinned_node_version(&repo)?;
    crate::node_pin::check_corpus(
        &pinned_node,
        &repo.join("conformance/vendor/node/manifest.json"),
    )?;
    ...
    println!();
    println!("node-suite (vendored Node {pinned_node} subset):");
    println!(
...
struct Scorecard<'a> {
    /// the Node the corpus was vendored from -- the pin in .node-version, which
    /// run() has already proven manifest.json agrees with.
    node_version: &'a str,
